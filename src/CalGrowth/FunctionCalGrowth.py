...
def openDB():
    # DB 열기
    with open(config.Calgrowth_path + 'DatabaseUser.json','r',encoding='UTF-8') as j_databaseUser:
        json_Userdatas = json.load(j_databaseUser)
    with open(config.Calgrowth_path + 'Database.json','r',encoding='UTF-8') as j_database:
        json_datas = json.load(j_database)
    with open(config.Calgrowth_path + 'TableExp.json','r',encoding='UTF-8') as j_tableexp:
        json_table_exp = json.load(j_tableexp)
    with open(config.Calgrowth_path + 'TableCredit.json','r',encoding='UTF-8') as j_tablecredit:
        json_table_credit = json.load(j_tablecredit)
    with open(config.Calgrowth_path + 'TableSkill.json','r',encoding='UTF-8') as j_tableskill:
...
def insertStudent(data, index, char_name, academy, mainoparts, suboparts):
    ...
    json_data = re.sub(r',\n\s+',',', json_data)
    json_data = re.sub(r'\n\s+\]',']', json_data)
    with open(config.Calgrowth_path + 'DatabaseUser.json', 'w',encoding='UTF-8') as f:
        f.write(json_data)
