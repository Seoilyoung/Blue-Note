...
from PyQt6.QtCore import Qt, QDate, QTimer, QSize, QEvent, QPoint, QUrl
from PyQt6.QtWidgets import (
    QMainWindow, QLabel, QWidget, QGraphicsDropShadowEffect, QAbstractItemView, QHeaderView,
    QTableWidgetItem, QListWidgetItem, QCompleter, QItemDelegate
)
from PyQt6.QtGui import QPixmap, QIcon, QFontMetrics, QCursor, QColor, QFontDatabase, QPainter, QDesktopServices
...
import src.CalGrowth as CalGrowth
import src.ApGuide.FunctionApGuide as ApGuide
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 재화계산화면
        self.json_Userdatas, self.json_datas, self.json_table_exp, self.json_table_credit, self.json_table_skill = CalGrowth.FunctionCalGrowth.openDB()
        self.db_list_char = CalGrowth.FunctionCalGrowth.readCharList(self.json_datas)
        ...
        self.calgrowth_layout(sorted_list, 'Character', config.container_char_path, self.listWidget_cal1)
        # class 생성 및 값 입력
        self.data_default = CalGrowth.DataUser.dataset('default')
        self.data_default.update(self.json_Userdatas)
        # 재화 layout 구성
        self.calgrowth_layout(self.list_oparts, 'Oparts', config.container_cal_path, self.listWidget_cal2)
        self.calgrowth_layout(self.list_academy, 'BD', config.container_cal_path, self.listWidget_cal3)
        self.calgrowth_layout(self.list_academy, 'Note', config.container_cal_path, self.listWidget_cal4)
    ...
    def calgrowth_layout(self, list_item, item_type, container_path, listwidget):
        ...
        for i in range(len(list_item)):
            ...
            if listwidget is self.listWidget_cal1:
                ...
                oparts_sub = CalGrowth.FunctionCalGrowth.readCharSubOparts(self.json_datas, char_name)
                memo = CalGrowth.FunctionCalGrowth.readCharMemo(self.json_Userdatas, char_name)
                #캐릭터 데이터 출력
                ...
                # 캐릭터별 필요 재화 계산
                CalGrowth.FunctionCalGrowth.calSkillTable(self.json_Userdatas, self.json_table_skill, self.json_datas, char_name)
    ...
    def material_layout(self):
        ...
        for (img_path, text), (label_img, label_name) in zip(config.etc_img_paths, labels):
    ...
    def calgrowth_insert(self):
        ...
        container_ui = loadUi(config.container_path)
        container = QListWidgetItem(listwidget)
        container.setSizeHint(QSize(0,98))
        container_ui.comboBox.wheelEvent = self.ignore_wheel_event
        container_ui.label_img.setScaledContents(True)
    ...
    def calgrowth_delete(self):
        ...
        if index >= 0:
            CalGrowth.FunctionCalGrowth.deleteStudent(self.json_Userdatas, index)
            self.json_Userdatas = CalGrowth.FunctionCalGrowth.openDBuser()
            # class 반영
            self.update_class()
        ...
        if container_ui is not None:
            ...
            if os.path.isfile(img_path):
                ...
                if char_name in self.json_Userdatas['Default']['Student'] and self.json_Userdatas['Default']['Student'][char_name]['index'] != row:
                    ...
                    CalGrowth.FunctionCalGrowth.initStudent(self.json_Userdatas, row, "Default "+str(row))
                    self.json_Userdatas = CalGrowth.FunctionCalGrowth.openDBuser()
                else:
                    CalGrowth.FunctionCalGrowth.updateStudent(self.json_Userdatas, row, char_name, academy, oparts_main, oparts_sub)
                    self.json_Userdatas = CalGrowth.FunctionCalGrowth.openDBuser()
    ...
    def on_text_changed(self):
        ...
        if container_ui is not None:
            ...
            CalGrowth.FunctionCalGrowth.updateMemo(self.json_Userdatas, row, char_name, memo)
    ...
    def eventFilter(self, source, event):
        if event.type() == QEvent.Type.Drop:
            ...
            if (current_row == self.listWidget_cal1.count()-1 and index.row()==-1) or (index.row() == current_row+1 and pos.y()%50<25):
                return True
            CalGrowth.FunctionCalGrowth.updateIndex(self.json_Userdatas, current_row, index.row())
    ...
    def on_table_cell_changed3(self):
            ...
            self.set_cell_colors(item_goal,item)
            CalGrowth.FunctionCalGrowth.updateTable2(self.json_Userdatas, item_type,'', cell_column, int(item.text()))
            self.json_Userdatas = CalGrowth.FunctionCalGrowth.openDBuser()
