...
# 함수 정의
def ContextTime(day,min):
    return (login_time-timedelta(days=day)+timedelta(minutes=min)).strftime('%Y/%m/%d %H시 %M분') + '접속\n'
@dispatch(int)
def CalAp(ap):
        return '보유 AP : ' + str(ap) + '\n'
@dispatch(int,int)
def CalAp(ap,ap_post):
        return '보유 AP : ' + str(ap) + ' / ' + str(ap_post) + '\n'


txt_title = event_name + ' AP 가이드'

txt_content_day[0] = ContextTime(3,120)
txt_content_day[0] += '  1. 카페 수령\n'
txt_content_day[0] += ContextTime(3,180)
txt_content_day[0] += '  1. 보유 AP 소모\n'
txt_content_day[0] += '\n\n'
ap_remain = 0
txt_content_ap += '\n\n\n'+ CalAp(ap_remain) + '\n\n\n'

txt_content_day[1] = ContextTime(2,60)
...
txt_content_day[1] += '\n\n'
ap_remain += 220
txt_content_ap += CalAp(ap_remain)
ap_remain += 150
txt_content_ap += CalAp(ap_remain)
ap_remain += 120*3
txt_content_ap += CalAp(ap_remain)
...
if ap_remain >1000:
    ...
    ap_remain -=ap_post
txt_content_ap += CalAp(ap_remain,ap_post) + '\n\n\n'

txt_content_day[2] = ContextTime(1,50)
...
txt_content_day[3] += ContextTime(0,0)
